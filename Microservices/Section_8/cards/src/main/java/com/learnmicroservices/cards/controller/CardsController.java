...
public class CardsController {
    ...
    public ResponseEntity<CardsDto> fetchCardInfo(
            @RequestHeader("demobank-correlation-id") String strCorrelationId,
            @RequestParam @Pattern(regexp = "(^$|[0-9]{10})",message = "Mobile Number must be 10 digit")
            String mobileNumber){

        logger.debug("Correlation demobank-correlation-id found in Cards Controller : {}",strCorrelationId);
